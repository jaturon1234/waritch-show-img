...
// 2. เงื่อนไข SQL: ดึงทุกอย่างที่เนื้อหาไม่ว่าง (เอา LIKE %img% ออกแล้ว)
$where_clause = "WHERE post_content IS NOT NULL AND post_content != ''";
        ...
        <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-12 gap-6 border-b border-gray-200 pb-8">
            <div>
                <nav class="flex text-sm text-gray-500 mb-2">
                    <a href="index.php" class="hover:text-blue-600 transition-colors">หน้าหลัก</a>
                ...
                <p class="text-gray-500 mt-2 text-lg">รายการข่าวสารทั้งหมด (พบ <?php echo $total_rows; ?> รายการ)</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    // ตรวจสอบหารูปภาพในเนื้อหา
                    if (preg_match('/<img.+?src=["\']([^"\']+\.(?i:jpg|jpeg|png|webp|gif))["\'].*?>/i', $post_content, $img_matches)) {
                        $img_src = $img_matches[1];
                    } else {
                        // หากไม่มีรูปภาพ ให้ใช้รูป Placeholder แทน
                        $img_src = 'assets/images/pdf_logo.png'; 
                    }
            ?>
            ...
            <?php
                ...
                echo '<div class="col-span-full py-20 text-center"><p class="text-gray-400 text-xl italic">ไม่พบรายการข่าวสารในขณะนี้...</p></div>';
            }
            ?>
        </div>
